...
import ChoiceMenu, { type ActionPanelMode } from "./ChoiceMenu"
...
const ADD_PLANET_HIT_AREAS = [
  { className: "absolute top-[60%] left-[44%] w-[8%]" },
  { className: "absolute top-[40%] left-[65%] w-[15%]" },
] as const

export default function PlanetScreen() {
  const [actionPanelMode, setActionPanelMode] = useState<ActionPanelMode | null>(null)

  const openChoiceMenu = () => setActionPanelMode("choose")
  ...
  return (
    ...
    >
      ...
      <PlanetMainMenu
        ...
        showAdd
      />
      {ADD_PLANET_HIT_AREAS.map((area) => (
        <button
          key={area.className}
          type="button"
          aria-label="Создать или присоединиться к миру"
          className={`${area.className} z-10 aspect-square cursor-pointer rounded-full`}
      ...
      {actionPanelMode && (
        <ChoiceMenu
          ...
          onClose={() => setActionPanelMode(null)}
          onModeChange={setActionPanelMode}
        />
      )}
    </div>
  )
