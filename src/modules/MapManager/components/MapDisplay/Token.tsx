...
export const Token = forwardRef<HTMLButtonElement, TokenProps>(function Token({ data, imageUrl, gridSize }, tokenRef) {
  ...
  const { xInWorld, yInWorld } = grid2world(data.pos_x ?? 0, data.pos_y ?? 0, gridSize)
  return (
    <PrestyledTooltip label={data.name} detail={`${data.current_hp}/${data.max_hp} HP`}>
      <Tooltip.Trigger
        ...
        className="absolute overflow-hidden rounded-full border-[0.2em] border-white shadow-[0_0.2em_0.5em_rgba(0,0,0,0.45)]"
        style={{
          backgroundColor: imageUrl ? undefined : data.color,
          width: `${tokenSize}px`,
          height: `${tokenSize}px`,
          left: `${xInWorld + (gridSize - tokenSize) / 2}px`,
          top: `${yInWorld + (gridSize - tokenSize) / 2}px`,
        }}
        ref={tokenRef}
      >
        {imageUrl ? (
